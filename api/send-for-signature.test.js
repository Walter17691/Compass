...
// same fix as send-letter.js's sibling test file: reproduces the
// live-verified exploit chain (a no-decision-authority case_access role
// delivering an unapproved dismissal letter) and asserts it's closed,
// while confirming legitimate meeting-record/reminder sends (no
// letterType, sometimes no caseId at all) are unaffected.
describe('send-for-signature — case linkage and outcome-approval gate (closes C2)', () => {
  let originalFetch;
  beforeEach(() => { originalFetch = global.fetch; });
  afterEach(() => { global.fetch = originalFetch; });
  ...
  it('403s a case_access holder with no decision role from sending a dismissal letter before approval', async () => {
    stubFetch({
  ...
  it('still allows a meeting-record signature request with no caseId at all (brand-new case not yet saved)', async () => {
    stubFetch({ members: [{ role: 'line_manager' }] });
    const res = mockRes();
  ...
  it('still allows a resend reminder for an existing case, with no letterType set', async () => {
    stubFetch({
      members: [{ role: 'line_manager' }],
    ...
    const res = mockRes();
    await handler(req({ ...body, caseId: 'case-1' }), res);
    expect(res.statusCode).toBe(200);
  });
});
