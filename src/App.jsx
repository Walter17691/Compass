...
const NO_INVENTED_AUTHORITIES = "Never cite a specific named tribunal case, decision, or legal citation — you cannot verify these are real. Discuss precedent in general terms only (e.g. \"tribunals have historically scrutinised...\"), never naming a specific case.";

// NEW-22 (final) — the evidential contract stopped Compass over-reading its
// own missing data, but it was never designed to stop a confident FALSE
// statement about the law. Human UAT reproduced exactly that: the Advisor
// Notes asserted "the ACAS Code of Practice requires that appeal hearings are
// recorded". It does not. That conflated three separate things — advisory
// record-keeping for cases, the genuine requirement to confirm the appeal
...
// NEW-22/23/25 — the post-meeting Review path (Meeting Summary, the Meeting
// Record's HR Advisor Notes, and the risk rating) had no epistemic contract at
// all. Asked for "legal risks" and a HIGH/MEDIUM/LOW rating from a transcript
// alone, it turned an employee's disputed assertion into a finding, an
// unverified historical attribution into a procedural defect, and both into a
// tribunal-outcome and compensation prediction.
//
// The boundary is EVIDENTIAL, not topical: where the record establishes a
// fact, the consequence may be stated; where it does not, the user is told
// what to check. Deterministic controls elsewhere (deadlines, the chair
// integrity trigger, letter validation, a recorded independence conflict) are
// untouched and must still state definite issues.
const REVIEW_EVIDENTIAL_CONTRACT = "EVIDENCE AND CERTAINTY — this governs everything you write here. Keep these apart and never collapse one into another: an ESTABLISHED FACT, directly established by reliable meeting or case information; an ATTRIBUTED STATEMENT, something a participant said or asserted; a DISPUTED MATTER, where accounts or positions differ; something UNRESOLVED or UNKNOWN, not yet established from the information available; and an INFERENCE, a cautious reading that must never be presented as fact."
  + " A statement by the employee is not automatically an established fact, and neither is a statement by a manager or the employer. A claim made during the meeting stays attributed to whoever made it unless it is independently established. Absence of evidence is not evidence of absence. Something being unknown, not recorded, not verified or unresolved does NOT mean there was a defect, a breach, a conflict, unfairness or non-compliance — it means it has not been established."
  + " Do not manufacture a contradiction or inconsistency merely because two statements differ in emphasis or wording; identify a tension only where the material genuinely supports one, and describe it neutrally."
  + " Do not predict what an employment tribunal would decide, whether a sanction or warning could be sustained, whether a dismissal would be found unfair, or what compensation or percentage uplift might follow. You may explain a general legal or procedural principle where it genuinely helps, but do not turn a general principle into a conclusion about THIS case unless the available facts actually establish the premise. " + NO_INVENTED_AUTHORITIES
  + " Where something matters but is not established, say what should be confirmed, checked, verified, established, reviewed, considered or recorded — do not decide it."
  + " " + LEGAL_ACCURACY_BOUNDARY;
...
Include all legally required elements. End with ## Next Steps checklist for HR.`,
  ...
  const summarizeMeetingsForCase = async (meetings) => {
    ...
    try {
      // NEW-24 — these cached summaries become buildCaseContext's body for
      // older meetings, so compressing the raw record here would
      // reintroduce HR Advisor commentary by the back door. Compress the
      // factual record only.
      const meetingsText = meetings.map(m => `MEETING id="${m.id}" — ${m.type||"Meeting"} on ${m.date||"unknown date"}\n${(stripAdvisorNotes(m.record)||"").slice(0, 3000)}`).join("\n\n");
      const res = await authedFetch("/api/chat", {method:"POST", headers:{"Content-Type":"application/json"}, body: JSON.stringify({
        model:"claude-sonnet-4-6",
        max_tokens:800,
        stream:false,
  ...
  const handleReview = async () => {
    ...
    try {
      ...
      fullRecord = await streamClaude(
        `You are a senior UK HR documentation specialist. Generate a meeting record with EXACTLY these three sections and NO others: ## Meeting Details (date, type, attendees, purpose), ## Meeting Dialogue (what was said, in concise prose), ## HR Advisor Notes (grounded UK HR procedural guidance in flowing prose - one paragraph covering relevant ACAS and procedural considerations, anything that needs verifying, and reasonable next steps). Do NOT add any other sections like Key Points, Next Steps, Summary, Actions, Risk Assessment or anything else. Three sections only. No bold, no emoji, no tables.${policies.length?" Reference company policies by name.":""} IMPORTANT: In the Meeting Dialogue section, prefix every line with initials only. Chair ${caseInfo.manager||"HR Manager"} = ${(caseInfo.manager||"HR Manager").split(" ").map(w=>w[0].toUpperCase()).join("")}. Employee ${caseInfo.employee||"Employee"} = ${(caseInfo.employee||"Employee").split(" ").map(w=>w[0].toUpperCase()).join("")}. Use ONLY these initials, never full names in the dialogue.\n\n${REVIEW_EVIDENTIAL_CONTRACT}\n\nThe HR Advisor Notes section follows that contract strictly. It may explain relevant procedural considerations, identify what needs verifying, explain why a confirmed fact would matter, and recommend reasonable next steps. It must NOT treat a participant's assertion as established, treat missing evidence as proof that something did not happen, or convert an unknown, disputed or unresolved matter into a defect, breach, conflict, unfairness or non-compliance. It must not say that a warning could not be sustained, that a sanction falls outside the band of reasonable responses, that a dismissal would be unfair, that a tribunal would reach any particular result, or that the organisation carries high, medium or low tribunal exposure, and it must not quantify compensation or any ACAS percentage uplift for this case. Where the information is insufficient, say what should be checked: "Check whether the vehicle policy was communicated to the employee before the incident and record the evidence relied upon", or "Consider whether the sanction remains appropriate once the disputed point has been resolved." If the meeting raised whether whoever is hearing an appeal was previously involved, treat that as a verification point and nothing more — Compass does not independently verify an officer's historical involvement from the structured record, incomplete historical attribution is not a conflict and not a procedural defect, and an appointment already made through the proper process does not need re-authorising here. A conflict genuinely recorded in the case record is different and should be stated plainly. NOTETAKER AND THE RECORD: a named notetaker is optional unless the authoritative information supplied establishes otherwise. "Notetaker: Not specified" means only that nobody has been named — it does NOT mean no notes were taken, that no adequate record exists, that the hearing was not appropriately recorded, that there is a procedural defect, or that there is a compliance concern. The meeting record you are producing here is itself a written record of this hearing, so never speculate about whether a record of the hearing exists, and never raise the absence of a named notetaker as a concern, a risk or something to verify. Genuine record concerns remain fully reportable where the supplied information establishes them — that the record is disputed, that it is inaccurate, that material information is missing from an existing record, or that the context affirmatively states no record was kept.`,
    ...
    try {
      ...
      const res = await authedFetch("/api/chat", {method:"POST", headers:{"Content-Type":"application/json"},
        body: JSON.stringify({model:"claude-sonnet-4-6", max_tokens:300, stream:false,
          system:'UK employment law risk specialist. Respond ONLY with valid JSON, no other text: {"rating":"<HIGH|MEDIUM|LOW>","summary":"two or three plain English sentences"} The rating field must be exactly one of HIGH, MEDIUM or LOW — replace the placeholder with whichever the record actually supports.'+' '+REVIEW_EVIDENTIAL_CONTRACT+' Rate only what the supplied record and context actually establish. An allegation, an assertion, a disputed point, or anything unknown, not recorded or not verified cannot on its own justify HIGH or MEDIUM — if the significant issues are still unresolved, say in the summary what needs verifying rather than asserting the adverse premise, and rate on what is established. Missing evidence is never proof of a procedural failure. RATING SCALE — rate the risk the available record ESTABLISHES, never the worst plausible scenario. HIGH: only where the record actually establishes a serious procedural or legal problem, or another serious matter with a clear and material potential consequence needing urgent HR or legal attention. A serious allegation that WOULD be significant if substantiated is not enough on its own. MEDIUM: where the record establishes a material issue needing HR attention that is not presently established as a serious legal or procedural failure. This can include established concerns alongside unresolved matters requiring verification — but unresolved matters are never treated as proven adverse facts. LOW: where the record does not establish a material procedural or legal problem and anything outstanding is routine, minor, or simply needs ordinary verification or follow-up. Do not default to LOW merely because matters are unresolved, do not default to MEDIUM or HIGH merely because an appeal exists, and do not reach for HIGH because an allegation would be serious if it turned out to be true. A genuinely established serious procedural or legal failure must still be rated HIGH.'+(historyContext?' If organisational history is provided, factor it into your rating — but keep organisation-wide patterns (how similar cases have been resolved before, across other employees) and this specific employee\'s own case history analytically separate. Never word the summary so it could be read as this employee personally having a prior warning or case unless the history explicitly says so for this employee by name — an unrelated case\'s outcome is base-rate context, not this person\'s record. Only suggest escalating up a disciplinary scale (e.g. "could escalate to a final warning") if this employee\'s own prior record actually supports that, not organisation-wide history alone.':'')+(policies.length?' If company policies are provided, factor any deviation from them into your rating too.':''),
  ...
  const runPrediction = async () => {
    ...
    try {
      ...
      await streamClaude(
        `UK employment tribunal outcome predictor. Analyse based on ERA 1996, ACAS Code, case law. Be honest about risks. You must NEVER recommend a specific sanction, disciplinary outcome, or final decision — describe risks and vulnerabilities only; the decision is HR's alone. ${NO_INVENTED_AUTHORITIES} ## headers.`,
