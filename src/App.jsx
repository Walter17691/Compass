...
{/* ══ CASE VIEW ══ */}
      {screen===SCREENS.CASE_VIEW&&activeCaseId&&(()=>{
        ...
        const stages = [
          {id:"investigation", label:"Investigation", meetings:invMeetings, hasEvidence:true, color:"#7C5CFC"},
          {id:"disciplinary", label:"Disciplinary", meetings:discMeetings, hasEvidence:false, color:"#C84B2F"},
          {id:"appeal", label:"Appeal", meetings:appealMeetings, hasEvidence:false, color:"#B87520"},
          ...(otherMeetings.length>0?[{id:"other", label:"Other meetings", meetings:otherMeetings, hasEvidence:false, color:"#6B6375"}]:[]),
        ].filter(s=>s.meetings.length>0||s.id==="investigation");

        const MeetingRow = ({m, caseObj}) => (
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"12px 0",borderBottom:"1px solid #F5F1EA",gap:12}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:6,flexShrink:0}}>
              {m.riskScore?.rating&&<span style={{fontSize:10,fontWeight:600,color:m.riskScore.rating==="HIGH"?"#C84B2F":"#B87520",background:m.riskScore.rating==="HIGH"?"#FEF0EB":"#FEF5E7",borderRadius:4,padding:"2px 7px"}}>{m.riskScore.rating}</span>}
              {m.signStatus==="signed"&&<span style={{fontSize:10,color:"#1A7A4A",background:"#E8F5EE",borderRadius:4,padding:"2px 7px",fontWeight:600}}>Signed</span>}
              {m.signStatus==="pending"&&<span style={{fontSize:10,color:"#B87520",background:"#FEF5E7",borderRadius:4,padding:"2px 7px"}}>Pending signature</span>}
              {m.signStatus==="pending"&&<button onClick={()=>saveCases(cases.map(x=>x.id===caseObj.id?{...x,meetings:x.meetings.map(mt=>mt.id===m.id?{...mt,signStatus:"signed"}:mt)}:x))}
                style={{fontSize:11,background:"#E8F5EE",border:"none",borderRadius:4,padding:"3px 8px",color:"#1A7A4A",cursor:"pointer",fontFamily:"DM Sans,system-ui,sans-serif"}}>Mark signed</button>}
              {m.record&&<button onClick={()=>{setReviewOutput(m.record);setMeetingType(MEETING_TYPES.find(t=>t.label===m.type)||null);setCaseInfo(p=>({...p,employee:caseObj.employeeName,manager:m.manager||"",date:m.date}));setScreen(SCREENS.REVIEW);}}
                style={{fontSize:11,background:"none",border:"1px solid #E8E0D0",borderRadius:6,padding:"4px 10px",color:"#6B6375",cursor:"pointer",fontFamily:"DM Sans,system-ui,sans-serif"}}>View notes</button>}
            </div>
          </div>
        );
        ...
        return(
          <div style={{minHeight:"100vh",background:"#FDFAF5",fontFamily:"DM Sans,system-ui,sans-serif",display:"flex",flexDirection:"column"}}>
            ...
            {/* Header */}
            <div style={{background:"#FFFFFF",borderBottom:"1px solid #EDE5D8",padding:"14px 28px",display:"flex",alignItems:"center",justifyContent:"space-between",flexShrink:0}}>
              <div style={{display:"flex",alignItems:"center",gap:12}}>
                <button onClick={()=>setScreen(SCREENS.CASES)}
                  style={{background:"none",border:"none",color:"#6B6375",fontSize:13,cursor:"pointer",fontFamily:"DM Sans,system-ui,sans-serif",padding:"4px 8px"}}>← Cases</button>
                <div style={{width:1,height:20,background:"#EDE5D8"}}/>
                <div>
                  <div style={{fontFamily:"DM Serif Display,Georgia,serif",fontSize:18,color:"#1A1535",fontWeight:400}}>{cs.employeeName}</div>
                  <div style={{fontSize:12,color:"#9B9098",display:"flex",gap:8,marginTop:1}}>
              ...
              <div style={{display:"flex",gap:8,alignItems:"center"}}>
                ...
                <button onClick={()=>{setMeetingSetup(p=>({...p,employee:cs.employeeName,manager:cs.manager||""}));setScreen(SCREENS.HOME+"_meeting");}}
            ...
            {/* Next action bar */}
            {nextStep&&cs.stage!=="closed"&&(
              <div style={{background:"#F5F3FF",borderBottom:"1px solid #DDD9F5",padding:"10px 28px",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
            ...
            {cs.stage==="closed"&&!showAppealInput[cs.id]&&!meetings.some(m=>(m.type||"").toLowerCase().includes("appeal"))&&(
              <div style={{background:"#E8F5EE",borderBottom:"1px solid #C8E6C9",padding:"10px 28px",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                <div style={{fontSize:13,color:"#1A7A4A",fontWeight:600}}>Case closed</div>
                <button onClick={()=>setShowAppealInput(p=>({...p,[cs.id]:true}))}
                  style={{fontSize:12,background:"none",border:"1px solid #C84B2F",borderRadius:6,padding:"5px 14px",color:"#C84B2F",cursor:"pointer",fontFamily:"DM Sans,system-ui,sans-serif"}}>
                  Employee is appealing
                </button>
              </div>
            )}
            {showAppealInput[cs.id]&&(
              <div style={{background:"#FEF5E7",borderBottom:"1px solid #F5E6C4",padding:"14px 28px"}}>
            ...
            <div style={{flex:1,display:"flex",overflow:"hidden"}}>
              ...
              <div style={{width:220,background:"#FFFFFF",borderRight:"1px solid #EDE5D8",padding:"20px 0",flexShrink:0,overflowY:"auto"}}>
                <div style={{padding:"0 16px",marginBottom:16}}>
                  <div style={{fontSize:11,fontWeight:600,color:"#9B9098",letterSpacing:"0.8px",textTransform:"uppercase"}}>Case stages</div>
                </div>
                {stages.map(s=>(
                  <button key={s.id} onClick={()=>setActiveCaseStage(s.id)}
                    style={{width:"100%",display:"flex",alignItems:"center",gap:10,padding:"10px 16px",background:activeCaseStage===s.id?"#F5F3FF":"none",border:"none",borderLeft:`3px solid ${activeCaseStage===s.id?s.color:"transparent"}`,cursor:"pointer",textAlign:"left",fontFamily:"DM Sans,system-ui,sans-serif"}}>
                    <div>
                      <div style={{fontSize:13,fontWeight:activeCaseStage===s.id?600:400,color:activeCaseStage===s.id?s.color:"#1A1535"}}>{s.label}</div>
                      <div style={{fontSize:11,color:"#9B9098",marginTop:1}}>{s.meetings.length} meeting{s.meetings.length!==1?"s":""}</div>
                    </div>
                  </button>
                ))}

                <div style={{margin:"16px",borderTop:"1px solid #EDE5D8",paddingTop:16}}>
                  <div style={{fontSize:11,fontWeight:600,color:"#9B9098",letterSpacing:"0.8px",textTransform:"uppercase",marginBottom:8}}>Case info</div>
                  {cs.description&&<div style={{fontSize:12,color:"#6B6375",lineHeight:1.5,marginBottom:8,fontStyle:"italic"}}>"{cs.description}"</div>}
                  {cs.referredBy&&<div style={{fontSize:11,color:"#9B9098"}}>Referred by: {cs.referredBy}</div>}
                  <button onClick={()=>{if(window.confirm("Delete this case?"))saveCases(cases.filter(x=>x.id!==cs.id));setScreen(SCREENS.CASES);}}
                    style={{marginTop:12,fontSize:11,color:"#C84B2F",background:"none",border:"none",cursor:"pointer",fontFamily:"DM Sans,system-ui,sans-serif",padding:0}}>
                    Delete case
                  </button>
                </div>
              </div>
              ...
              <div style={{flex:1,overflowY:"auto",padding:"24px 28px"}}>
                {stages.filter(s=>s.id===activeCaseStage).map(s=>(
                  <div key={s.id}>
                    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:20}}>
                      <div>
                        <h2 style={{fontFamily:"DM Serif Display,Georgia,serif",fontSize:20,color:"#1A1535",margin:0,fontWeight:400}}>{s.label}</h2>
                        <div style={{fontSize:12,color:"#9B9098",marginTop:2}}>{s.meetings.length} meeting{s.meetings.length!==1?"s":""}{s.id==="investigation"&&(cs.evidence||[]).length>0?" · "+(cs.evidence||[]).length+" evidence item"+((cs.evidence||[]).length!==1?"s":""):""}</div>
                      </div>
                      <button onClick={()=>{
                        const type = s.id==="investigation"?"investigation":s.id==="appeal"?"appeal-disciplinary":"disciplinary";
                        setMeetingSetup(p=>({...p,employee:cs.employeeName,manager:cs.manager||"",type}));
                        setScreen(SCREENS.HOME+"_meeting");
                      }} style={{fontSize:12,background:"none",border:"1px solid #7C5CFC",borderRadius:6,padding:"6px 14px",color:"#7C5CFC",cursor:"pointer",fontWeight:500,fontFamily:"DM Sans,system-ui,sans-serif"}}>
                        + Add meeting
                      </button>
                    </div>
                    ...
                    {/* Meetings */}
                    {s.meetings.length===0&&(
                      <div style={{textAlign:"center",padding:"40px 20px",background:"#FFFFFF",borderRadius:10,border:"1px solid #E8E0D0",marginBottom:16}}>
                        <div style={{fontSize:13,color:"#9B9098",marginBottom:12}}>No {s.label.toLowerCase()} meetings yet</div>
                        <button onClick={()=>{
                          const type = s.id==="investigation"?"investigation":s.id==="appeal"?"appeal-disciplinary":"disciplinary";
                          setMeetingSetup(p=>({...p,employee:cs.employeeName,manager:cs.manager||"",type}));
                          setScreen(SCREENS.HOME+"_meeting");
                        }} style={{fontSize:12,background:"#7C5CFC",border:"none",borderRadius:6,padding:"7px 16px",color:"#fff",cursor:"pointer",fontFamily:"DM Sans,system-ui,sans-serif"}}>
                          Start meeting
                        </button>
                      </div>
                    )}
                    {s.meetings.length>0&&(
                      <div style={{background:"#FFFFFF",border:"1px solid #E8E0D0",borderRadius:10,marginBottom:16,overflow:"hidden"}}>
                        <div style={{padding:"10px 16px",background:"#FDFAF5",borderBottom:"1px solid #EDE5D8"}}>
                          <div style={{fontSize:11,fontWeight:600,color:s.color,letterSpacing:"0.5px",textTransform:"uppercase"}}>Meetings</div>
                        </div>
                        <div style={{padding:"0 16px"}}>
                          {s.meetings.map((m,i)=><MeetingRow key={m.id||i} m={m} caseObj={cs}/>)}
                        </div>
                      </div>
                    )}

                    {/* Investigation bundle extras */}
                    {s.id==="investigation"&&(
                      <>
                        {/* Evidence */}
                        <div style={{background:"#FFFFFF",border:"1px solid #E8E0D0",borderRadius:10,marginBottom:16,overflow:"hidden"}}>
                          <div style={{padding:"10px 16px",background:"#FDFAF5",borderBottom:"1px solid #EDE5D8",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                            <div style={{fontSize:11,fontWeight:600,color:"#7C5CFC",letterSpacing:"0.5px",textTransform:"uppercase"}}>Evidence & witness statements</div>
                          </div>
                          <div style={{padding:"16px"}}>
                                ...
                                <div style={{flex:1,minWidth:0}}>
                                  ...
                                  <div style={{display:"flex",gap:6,marginTop:2,alignItems:"center",flexWrap:"wrap"}}>
                                    ...
                                    {ev.type==="Witness statement"&&(ev.signStatus==="signed"
                                      ...
                                      :<span style={{fontSize:10,color:"#B87520",background:"#FEF5E7",borderRadius:4,padding:"1px 6px"}}>Pending signature</span>
                                    )}
                                  </div>
                                </div>
                            ...
                            <label style={{display:"flex",alignItems:"center",justifyContent:"center",border:"2px dashed #E8E0D0",borderRadius:8,padding:"16px",cursor:"pointer",background:"#FDFAF5",marginTop:12,transition:"all 0.15s"}}
                              ...
                              onDrop={e=>{
                                e.preventDefault();e.currentTarget.style.borderColor="#E8E0D0";e.currentTarget.style.background="#FDFAF5";
                                Array.from(e.dataTransfer.files).forEach(f=>{
                                  const reader=new FileReader();
                                  reader.onload=ev=>{const newEv={name:f.name,type:f.type||"Document",size:f.size,date:new Date().toLocaleDateString("en-GB"),addedBy:currentUser?.name||"HR Manager",dataUrl:ev.target.result};saveCases(cases.map(x=>x.id===cs.id?{...x,evidence:[...(x.evidence||[]),newEv]}:x));};
                            ...
                            <div style={{marginTop:12,padding:"12px",background:"#F5F3FF",borderRadius:8,display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                              <div>
                                ...
                                <div style={{fontSize:11,color:"#9B9098"}}>Record and save to this investigation</div>
                              </div>
                              <button onClick={()=>{setMeetingSetup(p=>({...p,employee:"",manager:cs.manager||"",type:"investigation",linkedCaseId:cs.id,linkedCaseName:cs.employeeName}));setScreen(SCREENS.HOME+"_meeting");}}
                        ...
                        <div style={{background:"#FFFFFF",border:"1px solid #E8E0D0",borderRadius:10,overflow:"hidden"}}>
                          <div style={{padding:"10px 16px",background:"#FDFAF5",borderBottom:"1px solid #EDE5D8",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                            <div style={{fontSize:11,fontWeight:600,color:"#7C5CFC",letterSpacing:"0.5px",textTransform:"uppercase"}}>Investigation report</div>
                            {cs.investigationReport&&<button onClick={()=>{setLetterOutput(cs.investigationReport);setScreen(SCREENS.LETTER);}}
                              style={{fontSize:11,color:"#7C5CFC",background:"#EDE8FF",border:"none",borderRadius:4,padding:"3px 10px",cursor:"pointer",fontFamily:"DM Sans,system-ui,sans-serif",fontWeight:500}}>View report</button>}
                          </div>
                          <div style={{padding:"16px"}}>
                            {cs.investigationReport
                              ?<div style={{fontSize:13,color:"#1A7A4A"}}>Report generated on {fmtDate(cs.investigationReportDate)}</div>
                              :<div style={{fontSize:13,color:"#9B9098"}}>No investigation report yet — complete the investigation meetings first, then generate the report from the Next action bar above.</div>
                            }
                          </div>
                        </div>
                      </>
        ...
        <div style={{minHeight:"100vh",background:"#FDFAF5",fontFamily:"DM Sans,system-ui,sans-serif"}}>
          ...
          <div style={{maxWidth:620,margin:"0 auto",padding:"40px 24px"}}>
            ...
            <button
              ...
              onClick={()=>{
                const typeLabel = {misconduct:"Misconduct",grievance:"Grievance",performance:"Performance",attendance:"Attendance",redundancy:"Redundancy",discrimination:"Discrimination",whistleblowing:"Whistleblowing",other:"HR Matter"}[intake.type]||"HR Matter";
                const procTitle = typeLabel+(intake.description?" — "+intake.description.slice(0,40)+(intake.description.length>40?"...":""):"")+" · "+new Date(intake.dateReceived).toLocaleDateString("en-GB",{month:"short",year:"numeric"});
                const newCase = {
        ...
        <div style={{minHeight:"100vh",background:"#FDFAF5",fontFamily:"DM Sans,system-ui,sans-serif"}}>
          <div style={{background:"#FFFFFF",borderBottom:"1px solid #EDE5D8",padding:"16px 32px",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
            <div>
              ...
              <p style={{fontSize:13,color:"#9B9098",margin:"2px 0 0"}}>
                {cases.filter(cs=>getCaseStage(cs)!=="closed").length} active · {cases.filter(cs=>getCaseStage(cs)==="closed").length} closed
              </p>
            </div>
            <div style={{display:"flex",gap:8}}>
              <button onClick={()=>{setIntake({employee:"",manager:"",issue:"",type:"",dateReceived:new Date().toISOString().split("T")[0],description:"",referredBy:"",urgent:false});setScreen(SCREENS.INTAKE);}}
          ...
          <div style={{maxWidth:860,margin:"0 auto",padding:"28px 24px"}}>
            ...
            {(()=>{
              ...
              return employees.map(emp=>{
                ...
                const activeProcCount = empCases.filter(cs=>getCaseStage(cs)!=="closed").length;
                return(
                  <div key={emp} style={{marginBottom:24}}>
                    ...
                    <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:10}}>
                      ...
                      <div>
                        <div style={{fontSize:16,fontWeight:600,color:"#1A1535"}}>{emp}</div>
                        <div style={{fontSize:12,color:"#9B9098"}}>{empCases.length} proceeding{empCases.length!==1?"s":""}{activeProcCount>0?" · "+activeProcCount+" active":""}</div>
                      </div>
                    </div>
                    ...
                    {empCases.map(cs=>{
                      ...
                      const title = getProceedingTitle(cs);
                      return(
                        <div key={cs.id} onClick={()=>{setActiveCaseId(cs.id);setActiveCaseStage("investigation");setScreen(SCREENS.CASE_VIEW);}}
                          ...
                          <div style={{flex:1,minWidth:0}}>
                            <div style={{fontSize:13,fontWeight:closed?400:600,color:closed?"#9B9098":"#1A1535",marginBottom:3}}>{title}</div>
                            <div style={{fontSize:11,color:"#9B9098",display:"flex",gap:8,flexWrap:"wrap"}}>
                              <span>{(cs.meetings||[]).length} meeting{(cs.meetings||[]).length!==1?"s":""}</span>
                              {cs.urgent&&<span style={{color:"#C84B2F",fontWeight:600}}>· URGENT</span>}
                            </div>
                            {next&&!closed&&(
                              <div style={{fontSize:11,color:"#7C5CFC",fontWeight:500,marginTop:4}}>Next: {next.label}</div>
                            )}
                          </div>
                          <div style={{display:"flex",alignItems:"center",gap:8,flexShrink:0}}>
                            <span style={{fontSize:11,fontWeight:600,color:getCaseStatus(cs).color,background:getCaseStatus(cs).bg,borderRadius:20,padding:"3px 10px"}}>{getCaseStatus(cs).label}</span>
