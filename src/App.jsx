...
Include: date, greeting, what was discussed, agreed outcomes, next steps, signature block.`,
  ...
  const doSend = async (action, sig) => {
    ...
    if(action==="download") {
      ...
      try { const d=await generatePDF(sig); d.save(`Compass_${meetingType?.label}_${caseInfo.employee||"Letter"}.pdf`); } catch(e){alert(e.message);}
      setPdfGenerating(false);
    } else {
      setPdfGenerating(true);
      try {
        const d=await generatePDF(sig); d.save(`Compass_Letter_${caseInfo.employee||"Letter"}.pdf`);
        const url = action==="gmail"
          ? `https://mail.google.com/mail/?view=cm&fs=1&to=${to}&su=${subj}&body=${encodeURIComponent("Please find the letter attached.\n\nGenerated by Compass HR.")}`
          : `https://outlook.office.com/mail/deeplink/compose?to=${to}&subject=${subj}&body=${encodeURIComponent("Please find the letter attached.\n\nGenerated by Compass HR.")}`;
  ...
  return (
    <div style={{minHeight:"100vh",background:"#0D0D0F",fontFamily:"Inter,system-ui,sans-serif",color:"#F2EDE4"}}>
      ...
      <header style={{background:"#0D0D0F",borderBottom:"1px solid #1C1C22",position:"sticky",top:0,zIndex:99}}>
        <div style={{maxWidth:1440,margin:"0 auto",padding:"10px 20px",display:"flex",alignItems:"center",justifyContent:"space-between",gap:12}}>
          ...
          <nav style={{display:"flex",alignItems:"center",gap:4,flexShrink:0}}>
            ...
            {[
              {s:SCREENS.DASHBOARD,l:"Dashboard"},
              {s:SCREENS.CASES,l:"Cases"+(cases.length?` (${cases.length})`:"")},
              {s:SCREENS.TEMPLATES,l:"Templates"},
              {s:SCREENS.WHISTLE,l:"Reports"},
              {s:SCREENS.PORTAL,l:"Portal"},
              {s:SCREENS.NEWSTARTER,l:"Starters"},
              {s:SCREENS.ERREPORT,l:"ER Analytics"},
            ...
            {/* Search */}
            <button onClick={()=>setScreen(SCREENS.SEARCH)} style={{background:screen===SCREENS.SEARCH?"#1C1C22":"none",border:"1px solid #2A2A35",color:"#666",borderRadius:6,padding:"5px 10px",fontSize:12}}>Search</button>
            {/* Deadline indicator */}
            {dueSoon.length>0&&(
              <button onClick={()=>setScreen(SCREENS.DASHBOARD)}
                style={{background:dueSoon.some(d=>d.overdue)?"#2A1008":"#1A1A08",border:"1px solid",borderColor:dueSoon.some(d=>d.overdue)?"#E8622A44":"#D4882A44",color:dueSoon.some(d=>d.overdue)?"#E8622A":"#D4882A",borderRadius:6,padding:"4px 10px",fontSize:11,fontWeight:600}}>
                {dueSoon.filter(d=>d.overdue).length>0?`${dueSoon.filter(d=>d.overdue).length} overdue`:`${dueSoon.length} due soon`}
              </button>
            )}
            {/* User */}
            ...
            {meetingType&&<button onClick={reset} style={{background:"none",border:"1px solid #2A2A35",color:"#555",borderRadius:6,padding:"4px 10px",fontSize:11}}>End</button>}
          </nav>
        </div>
      </header>
      ...
      {/* ══ HOME ══ */}
      {screen===SCREENS.HOME&&(
        <div style={{maxWidth:1000,margin:"0 auto",padding:"60px 20px 80px"}}>
          <div style={{textAlign:"center",marginBottom:48}}>
            <div style={{display:"flex",justifyContent:"center",marginBottom:20}}><CompassLogo size={64}/></div>
            ...
            <h1 style={{fontFamily:"Playfair Display,Georgia,serif",fontSize:"clamp(32px,5vw,60px)",fontWeight:400,color:"#F2EDE4",margin:"0 0 14px",lineHeight:1.1}}>
              Navigate every<br/><em style={{color:"#7C5CFC"}}>HR conversation.</em>
            </h1>
            <p style={{fontSize:15,color:"#888",maxWidth:480,margin:"0 auto",lineHeight:1.7}}>
              AI-powered meeting records, legal risk scoring, outcome letters, and full case management — built for UK HR professionals.
            </p>
          </div>
          <div style={{fontSize:10,letterSpacing:2,textTransform:"uppercase",color:"#444",fontWeight:600,marginBottom:12}}>Formal meetings</div>
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(160px,1fr))",gap:8,marginBottom:24}}>
            {MEETING_TYPES.filter(t=>t.group==="formal").map(t=>(
              <button key={t.id} onClick={()=>startSession(t)}
                style={{background:"#1C1C22",border:"1px solid #2A2A35",borderRadius:10,padding:"18px 16px",display:"flex",flexDirection:"column",gap:10,textAlign:"left",transition:"all 0.15s"}}
                ...
                <div style={{fontFamily:"Playfair Display,Georgia,serif",fontSize:14,color:"#F2EDE4",fontWeight:600,lineHeight:1.3}}>{t.label}</div>
                <div style={{fontSize:11,color:"#7C5CFC",fontWeight:600}}>Begin →</div>
              </button>
            ))}
          </div>

          <div style={{fontSize:10,letterSpacing:2,textTransform:"uppercase",color:"#444",fontWeight:600,marginBottom:12}}>Development meetings</div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8}}>
            {[
              {s:SCREENS.DASHBOARD,l:"Dashboard",sub:"Case overview & risk trends"},
              {s:SCREENS.TEMPLATES,l:"Template library",sub:"UK-compliant HR letters"},
              {s:SCREENS.WHISTLE,l:"Whistleblower intake",sub:"Anonymous reporting portal"},
              {s:SCREENS.NEWSTARTER,l:"New starter onboarding",sub:"AI-assisted induction journeys"},
              {s:SCREENS.ERREPORT,l:"ER analytics",sub:"Case patterns & trend reporting"},
              {s:SCREENS.REDUNDANCY,l:"Redundancy & consultation",sub:"Individual & collective processes"},
              {s:SCREENS.WELLBEING,l:"Mental health & wellbeing",sub:"Confidential wellbeing case notes"},
            ].map(({s,l,sub})=>(
              <button key={s} onClick={()=>setScreen(s)}
                style={{background:"#1C1C22",border:"1px solid #2A2A35",borderRadius:10,padding:"16px",textAlign:"left",cursor:"pointer",transition:"border-color 0.15s"}}
                onMouseEnter={e=>{e.currentTarget.style.borderColor="#7C5CFC44";}}
                onMouseLeave={e=>{e.currentTarget.style.borderColor="#2A2A35";}}>
                <div style={{fontSize:13,color:"#F2EDE4",fontWeight:600,marginBottom:3}}>{l}</div>
                <div style={{fontSize:11,color:"#555"}}>{sub}</div>
              </button>
            ))}
          </div>
          <p style={{fontSize:11,color:"#333",textAlign:"center",marginTop:32,lineHeight:1.6}}>Compass provides AI-assisted guidance. Always verify against current UK employment law.</p>
        </div>
      )}
      ...
      {screen===SCREENS.PREP&&(
        <div style={{maxWidth:1280,margin:"0 auto",padding:"32px 20px"}}>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:20,alignItems:"start"}}>
            <Card>
              <h2 style={{fontFamily:"Playfair Display,Georgia,serif",fontSize:20,color:"#7C5CFC",margin:"0 0 4px",fontWeight:600}}>Prepare for {meetingType?.label}</h2>
              <p style={{fontSize:12,color:"#666",margin:"0 0 20px"}}>Fill in case details. Compass will build a preparation pack referencing UK law and your policies.</p>

              {/* Case details */}
              {[{k:"employee",l:"Employee name",req:true,ph:"e.g. Sarah Johnson"},{k:"email",l:"Email",ph:"sarah@company.com",type:"email"},{k:"manager",l:"Chair / HR manager",ph:"Your name"},{k:"date",l:"Meeting date",type:"date"}].map(f=>(
                <div key={f.k} style={{marginBottom:14}}>
                  <label style={{display:"block",fontSize:10,fontWeight:600,color:"#666",letterSpacing:0.8,textTransform:"uppercase",marginBottom:5}}>{f.l}{f.req&&<span style={{color:"#E8622A"}}> *</span>}</label>
                  <input type={f.type||"text"} placeholder={f.ph} value={caseInfo[f.k]} onChange={e=>setCaseInfo(p=>({...p,[f.k]:e.target.value}))}
                    style={{width:"100%",background:"#0D0D0F",border:"1px solid #2A2A35",borderRadius:6,padding:"9px 12px",fontSize:13,outline:"none"}} />
                </div>
              ))}
              <div style={{marginBottom:18}}>
                <label style={{display:"block",fontSize:10,fontWeight:600,color:"#666",letterSpacing:0.8,textTransform:"uppercase",marginBottom:5}}>Background</label>
                <textarea value={caseInfo.context} onChange={e=>setCaseInfo(p=>({...p,context:e.target.value}))} placeholder="Brief context — previous warnings, allegations, history..."
                  style={{width:"100%",background:"#0D0D0F",border:"1px solid #2A2A35",borderRadius:6,padding:"9px 12px",fontSize:13,outline:"none",height:80,resize:"vertical"}} ></textarea>
              </div>

              {/* Participants */}
              <div style={{marginBottom:18}}>
                <label style={{display:"block",fontSize:10,fontWeight:600,color:"#666",letterSpacing:0.8,textTransform:"uppercase",marginBottom:8}}>Meeting participants</label>
                {participants.map((p,i)=>(
                  <div key={i} style={{display:"grid",gridTemplateColumns:"1fr 1fr auto",gap:6,marginBottom:6}}>
                    <input placeholder="Name" value={p.name} onChange={e=>setParticipants(ps=>ps.map((x,j)=>j===i?{...x,name:e.target.value}:x))}
                      style={{background:"#0D0D0F",border:"1px solid #2A2A35",borderRadius:6,padding:"7px 10px",fontSize:12,outline:"none",color:"#F2EDE4"}} />
                    <input placeholder="Role (e.g. Union Rep)" value={p.role} onChange={e=>setParticipants(ps=>ps.map((x,j)=>j===i?{...x,role:e.target.value}:x))}
                      style={{background:"#0D0D0F",border:"1px solid #2A2A35",borderRadius:6,padding:"7px 10px",fontSize:12,outline:"none",color:"#F2EDE4"}} />
                    <button onClick={()=>setParticipants(ps=>ps.filter((_,j)=>j!==i))} style={{background:"none",border:"1px solid #2A2A35",borderRadius:6,padding:"7px 10px",color:"#E8622A",fontSize:12}}>✕</button>
                  </div>
                ))}
                <button onClick={()=>setParticipants(p=>[...p,{name:"",role:"",email:""}])}
                  style={{background:"none",border:"1px dashed #2A2A35",borderRadius:6,padding:"7px 12px",fontSize:12,color:"#666",width:"100%"}}>
                  + Add participant
                </button>
              </div>

              <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
                <Btn onClick={handlePrepare} disabled={aiProcessing||!caseInfo.employee.trim()}>{aiProcessing?"Preparing...":"Generate prep pack"}</Btn>
                <Btn variant="ghost" onClick={()=>setScreen(SCREENS.RECORD)}>Skip to recording</Btn>
              </div>
            </Card>

            <Card style={{background:"#141418",minHeight:300}}>
              {!prepNotes&&!aiProcessing&&(
                <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:200,gap:10}}>
                  <CompassLogo size={36}/>
                  <div style={{fontSize:13,color:"#555"}}>Preparation pack will appear here</div>
                  <div style={{fontSize:11,color:"#444"}}>Agenda · Questions · Legal checklist · Risk flags</div>
                </div>
              )}
              {aiProcessing&&!prepNotes&&<div style={{textAlign:"center",padding:40}}><span className="pu" style={{color:"#7C5CFC",fontSize:24}}>●</span><div style={{color:"#666",marginTop:10,fontSize:13}}>Building prep pack...</div></div>}
              {prepNotes&&(
                <>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
                    <SectionTitle>PREP PACK</SectionTitle>
                    <Btn variant="ghost" onClick={()=>navigator.clipboard.writeText(prepNotes)} style={{padding:"3px 10px",fontSize:11}}>Copy</Btn>
                  </div>
                  <MDRenderer text={prepNotes}/>
                  {!aiProcessing&&<Btn onClick={()=>setScreen(SCREENS.RECORD)} style={{marginTop:16}}>Start recording →</Btn>}
                </>
              )}
            </Card>
          </div>
              ...
              {whistleSubmitted?(
                ...
                <Card>
                  ...
                  <div style={{marginBottom:14}}>
                    ...
                    <input type="date" value={whistleForm.date} onChange={e=>setWhistleForm(p=>({...p,date:e.target.value}))}
                      style={{width:"100%",background:"#0D0D0F",border:"1px solid #2A2A35",borderRadius:6,padding:"9px 12px",fontSize:13,outline:"none",color:"#F2EDE4"}} />
                  </div>
                  <div style={{marginBottom:16}}>
              ...
              <Card style={{marginBottom:20}}>
                ...
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,marginBottom:14}}>
                  ...
                  <div>
                    ...
                    <input type="date" value={wellbeingForm.date} onChange={e=>setWellbeingForm(p=>({...p,date:e.target.value}))}
                      style={{width:"100%",background:"#0D0D0F",border:"1px solid #2A2A35",borderRadius:6,padding:"9px 12px",fontSize:13,color:"#F2EDE4",outline:"none",boxSizing:"border-box"}} />
                  </div>
                  <div>
                ...
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,marginBottom:14}}>
                  ...
                  <div>
                    ...
                    <input type="date" value={wellbeingForm.followUpDate} onChange={e=>setWellbeingForm(p=>({...p,followUpDate:e.target.value}))}
                      style={{width:"100%",background:"#0D0D0F",border:"1px solid #2A2A35",borderRadius:6,padding:"9px 12px",fontSize:13,color:"#F2EDE4",outline:"none",boxSizing:"border-box"}} />
                  </div>
                </div>
                <div style={{display:"flex",gap:10}}>
