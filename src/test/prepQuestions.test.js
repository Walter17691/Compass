...
// Advisory accuracy P1 (Human UAT, 2026-09-20) — the markdown pack carried a
// narrative "## Key Questions" section duplicating the interactive question
// cards, which are generated by a separate call from identical inputs and are
// the editable, linkable, trackable version. The narrative section was removed
// from the required section list; the card architecture is untouched.
describe('narrative section list vs interactive question cards (11, 12)', () => {
  // vitest root is the repo root.
  const appSource = readFileSync('src/App.jsx', 'utf8');
  const sectionList = (appSource.match(/## Objectives[^`]*## Risk Flags/) || [])[0] || '';

  it('11. the required narrative sections no longer include Key Questions', () => {
    expect(sectionList).toBeTruthy();
    expect(sectionList).not.toContain('## Key Questions');
  });

  it('11. every other required narrative section is retained', () => {
    ['## Objectives', '## Agenda', '## Opening Script', '## Evidence to Explore',
     '## Unanswered Issues', '## Potential Inconsistencies', '## Closing Points',
     '## Legal Checklist', '## Risk Flags'].forEach(section => {
      expect(sectionList).toContain(section);
    });
  });

  it('12. the structured question generator and its category schema remain intact', () => {
    expect(appSource).toContain('const generatePrepQuestions = async (carriedContext, prepInstructions)');
    expect(appSource).toMatch(/"agenda"\|\\?"evidence\\?"\|\\?"clarification\\?"\|\\?"unanswered\\?"|agenda.*evidence.*clarification.*unanswered/);
    expect(appSource).toContain('generatePrepQuestions(carriedContext, prepInstructions)');
  });

  it('12. the interactive card behaviours are still wired', () => {
    const prepScreen = readFileSync('src/screens/PrepScreen.jsx', 'utf8');
    expect(prepScreen).toContain('PrepQuestionRow');
    expect(prepScreen).toContain('onTogglePrepQuestionEssential');
    expect(prepScreen).toContain('onMovePrepQuestion');
    expect(prepScreen).toContain('onRemovePrepQuestion');
    expect(prepScreen).toContain('onLinkPrepQuestionToAllegation');
    expect(prepScreen).toContain('onLinkPrepQuestionToEvidence');
    expect(prepScreen).toContain('Why ask this?');
  });
});
