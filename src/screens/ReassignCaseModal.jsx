...
export function ReassignCaseModal({ cases, activeCaseId, currentUser, orgMembers, selectedMemberId, setSelectedMemberId, setShowReassignModal, saveCases, org, user, showToast, audit }) {
  ...
  const close = () => { setShowReassignModal(false); setSelectedMemberId(""); };
  const containerRef = useRef(null);
  useModalA11y(containerRef, close);
  return (
    <div role="dialog" aria-modal="true" aria-labelledby="reassign-case-title" ref={containerRef} tabIndex={-1} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
      <div style={{background:"#FFFFFF",borderRadius:16,padding:28,width:"100%",maxWidth:480}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:6}}>
          <div id="reassign-case-title" style={{fontFamily:"DM Serif Display,Georgia,serif",fontSize:20,color:"#1C1820"}}>Reassign case</div>
        ...
        {eligible.length===0?(
          ...
          <>
            <div style={{marginBottom:16}}>
              <label htmlFor="reassign-case-owner" style={{fontSize:12,fontWeight:600,color:"#1C1820",display:"block",marginBottom:6}}>New case owner</label>
              <select id="reassign-case-owner" value={selectedMemberId||eligible[0]?.id||""} onChange={e=>setSelectedMemberId(e.target.value)} style={{width:"100%",fontSize:13,border:"1px solid #E8E0D0",borderRadius:8,padding:"10px 12px",background:"#fff",color:"#1C1820"}}>
