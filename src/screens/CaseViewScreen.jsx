    ...
    <div style={{minHeight:"100vh",background:"#FDFAF5",fontFamily:"DM Sans,system-ui,sans-serif",display:"flex",flexDirection:"column"}}>
      ...
      <div style={{flex:1,overflowY:"auto",padding:"24px 28px"}}>
        <div style={{maxWidth:800,margin:"0 auto"}}>
          {activeTab==="overview"&&(
            <OverviewTab cs={cs} cases={cases} saveCases={saveCases} stage={stage} currentRisk={currentRisk} empRecord={empRecord} repeatCount={repeatCount} confirmDialog={confirmDialog} setScreen={setScreen} screens={screens} caseSignals={caseSignals} caseTasks={caseTaskList} unansweredCovered={unansweredCovered} unansweredLoading={unansweredLoading} generateUnansweredQuestions={generateUnansweredQuestions} createCaseTask={createCaseTask} changeSignalStatus={changeSignalStatus} onAskWhy={setWhySignal} allegations={allegations} generateInconsistencies={generateInconsistencies} inconsistencyLoading={inconsistencyLoading?.[cs.id]} linkSignalToAllegation={linkSignalToAllegation} requestOverrideReason={requestOverrideReason} requestPolicyDeviationReason={requestPolicyDeviationReason} caseAccess={caseAccess} orgMembers={orgMembers} assignCaseRole={assignCaseRole} hrReviewRequests={hrReviewRequests} respondToReview={respondToReview} resolveInvestigationReview={resolveInvestigationReview} isApprover={isHR} auditLog={auditLog} wellbeingNotes={wellbeingNotes} dueSoon={dueSoon} processTemplates={processTemplates}/>
          )}
          {activeTab==="timeline"&&(
          ...
          {activeTab==="documents"&&(
            <DocumentsTab cs={cs} setLetterOutput={setLetterOutput} setScreen={setScreen} screens={screens} fmtDate={fmtDate} onGenerateHearingPack={onGenerateHearingPack} hearingPackGenerating={!!hearingPackGenerating?.[cs.id]}/>
          )}
          {activeTab==="outcome"&&(
