  ...
  return (
    <div style={{background:"#FFFFFF",border:"1px solid #E8E0D0",borderRadius:12,overflow:"hidden"}}>
      <div style={{padding:"12px 16px",background:"#FDFAF5",borderBottom:"1px solid #EDE5D8",display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:8}}>
        <div style={{fontSize:11,fontWeight:700,color:"#7C5CFC",letterSpacing:"0.5px",textTransform:"uppercase"}}>Themes ({applied.length})</div>
        <button onClick={()=>onSuggest(cs)} disabled={suggesting} style={{fontSize:12,background:"#7C5CFC",border:"none",borderRadius:8,padding:"6px 14px",color:"#fff",fontWeight:600,cursor:suggesting?"default":"pointer",opacity:suggesting?0.6:1,fontFamily:"DM Sans,system-ui,sans-serif"}}>
      ...
      <div style={{padding:"16px"}}>
          ...
          {applied.map(t => {
            ...
            return (
              <span key={t.id} style={{display:"flex",alignItems:"center",gap:6,fontSize:12,color:"#1A1535",background:"#F5F3FF",border:"1px solid #E0D8FF",borderRadius:20,padding:"4px 6px 4px 12px"}}>
                {theme.name}
                <button onClick={()=>onRemove(t.id)} aria-label={"Remove "+theme.name} style={{background:"none",border:"none",color:"#9B9098",cursor:"pointer",fontSize:13,padding:"0 4px",fontFamily:"DM Sans,system-ui,sans-serif"}}>×</button>
              </span>
            );
          })}
        </div>
        ...
        {pendingSuggestions.length>0 && (
          <div style={{marginBottom:16,paddingBottom:16,borderBottom:"1px solid #F5F1EA"}}>
            <div style={{fontSize:10,fontWeight:700,color:"#9B9098",letterSpacing:"0.5px",textTransform:"uppercase",marginBottom:8}}>AI-suggested themes</div>
                ...
                return (
                  <div key={name} style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:8,background:"#FDFAF5",border:"1px solid #E8E0D0",borderRadius:8,padding:"8px 12px"}}>
                    ...
                    <div style={{display:"flex",gap:6,flexShrink:0}}>
                      <button onClick={()=>onConfirmSuggestion(cs, name)} disabled={!canConfirm} style={{fontSize:11,background:canConfirm?"#1A7A4A":"#E8E0D0",border:"none",borderRadius:6,padding:"4px 10px",color:"#fff",fontWeight:600,cursor:canConfirm?"pointer":"default",fontFamily:"DM Sans,system-ui,sans-serif"}}>Confirm</button>
                      <button onClick={()=>onDismissSuggestion(cs, name)} style={{fontSize:11,background:"none",border:"1px solid #E8E0D0",borderRadius:6,padding:"4px 10px",color:"#6B6375",cursor:"pointer",fontFamily:"DM Sans,system-ui,sans-serif"}}>Dismiss</button>
                    </div>
                  </div>
                );
              })}
        ...
        {available.length>0 && (
          <div style={{display:"flex",gap:8,alignItems:"center"}}>
            ...
            <button onClick={()=>{ if(pickerValue){ onAssignExisting(cs, pickerValue); setPickerValue(""); } }} disabled={!pickerValue} style={{fontSize:12,background:"none",border:"1px solid #E8E0D0",borderRadius:8,padding:"6px 12px",color:"#1A1535",cursor:pickerValue?"pointer":"default",opacity:pickerValue?1:0.5,fontFamily:"DM Sans,system-ui,sans-serif"}}>Add</button>
          </div>
        )}
      </div>
    </div>
