...
export function ActivityBell({ auditLog, orgId }) {
  ...
  return (
    <div style={{position:"relative"}} ref={ref}>
      <button ref={btnRef} onClick={toggle} aria-label={`Activity${unreadCount?` (${unreadCount} unread)`:""}`} style={{position:"relative",background:show?"#F5F3FF":"none",border:"1px solid #E8E0D0",borderRadius:6,padding:"5px 10px",fontSize:13,cursor:"pointer",color:"#6B6375"}}>
      ...
      {show&&popoverStyle&&(
        <div role="menu" aria-label="Recent activity" style={{...popoverStyle,background:"#FFFFFF",border:"1px solid #E8E0D0",borderRadius:8,boxShadow:"0 8px 24px rgba(0,0,0,0.12)",width:320,maxWidth:"calc(100vw - 24px)",overflowY:"auto",zIndex:250}}>
