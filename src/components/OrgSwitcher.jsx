...
export function OrgSwitcher({ org, availableOrgs=[], switchOrg, onJoinAnotherOrg }) {
  ...
  return (
    <div style={{position:"relative"}} ref={ref}>
      <button ref={btnRef} onClick={()=>setShow(v=>!v)} aria-label={`Switch organisation (current: ${org.name})`} style={{fontSize:11,color:"#9B9098",background:"#F5F1EA",border:"none",borderRadius:4,padding:"3px 8px",cursor:"pointer",display:"flex",alignItems:"center",gap:4}}>
        {org.name}
        {availableOrgs.length>1&&<span style={{fontSize:9}}>▾</span>}
      </button>
      {show&&popoverStyle&&(
        <div role="menu" aria-label="Organisations" style={{...popoverStyle,background:"#FFFFFF",border:"1px solid #E8E0D0",borderRadius:8,boxShadow:"0 8px 24px rgba(0,0,0,0.12)",minWidth:220,overflowY:"auto",zIndex:250}}>
          {availableOrgs.map(o=>(
