...
const BarRow = ({ label, value, max, color = COLOR.inkQuiet, onViewCases }) => (
...
// RPC/table needed.
export function CaseQualityAnalyticsPanel({ cases, allegations, caseSignals, caseTasks, policies, caseAccess, orgMembers, onViewCases }) {
  ...
  return (
    <div>
      ...
      {topIssues.length === 0
        ...
        : topIssues.map(issue => <BarRow key={issue.id} label={issue.label} value={issue} max={max} onViewCases={onViewCases ? () => onViewCases({ caseIds: issue.caseIds }) : null}/>)}
    </div>
  );
}
