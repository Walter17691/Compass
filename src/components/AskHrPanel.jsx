...
// 10/10 pass, Part A — no longer its own card; composes as a small
// Layer 3 subsection (Ask HR is a side-channel question thread, not a
// case-progression blocker, per the Overview redesign's own Layer
// classification). Logic/handlers unchanged.
export function AskHrPanel({ cs, hrReviewRequests, respondToReview, isHR }) {
  ...
  return (
    <div>
      <div style={{...TYPE.sectionHeading,color:COLOR.inkFaint,marginBottom:SPACE.sm}}>Ask HR</div>
        ...
        {requests.map((r,i)=>(
          <div key={r.id} style={{padding:"10px 0",borderBottom:i<requests.length-1?`1px solid ${COLOR.borderFaint}`:"none"}}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:10,flexWrap:"wrap"}}>
              <span style={{fontSize:13,fontWeight:600,color:COLOR.ink}}>{r.requested_by_name?r.requested_by_name+" asked for help":"HR review requested"}</span>
              <span style={{fontSize:11,fontWeight:700,color:r.status==="pending"?COLOR.amber:COLOR.green}}>{r.status==="pending"?"Awaiting HR response":"Resolved"}</span>
            </div>
            {r.record_snapshot&&<div style={{fontSize:12,color:COLOR.inkSoft,marginTop:6,whiteSpace:"pre-wrap"}}>{r.record_snapshot}</div>}
            {r.status==="pending"&&isHR&&(
              <div style={{marginTop:8}}>
                <button onClick={()=>respondToReview(r.id,"resolved")} style={{fontSize:11,color:"#FFFFFF",background:COLOR.green,border:"none",borderRadius:5,padding:"5px 12px",cursor:"pointer",fontFamily:"DM Sans,system-ui,sans-serif",fontWeight:600}}>Mark resolved</button>
              </div>
            )}
            {r.status!=="pending"&&r.reviewed_by_name&&<div style={{fontSize:11,color:COLOR.inkFaint,marginTop:6}}>By {r.reviewed_by_name}</div>}
          </div>
        ))}
      </div>
    </div>
