...
export default function PortalSignup({ onLogin }) {
  ...
  const inp = (extra = {}) => ({
    ...
    fontFamily: "Archivo, system-ui, sans-serif", transition: "border-color 0.15s", ...extra
  })
  const label = { fontSize: 12, fontWeight: 600, color: C.muted, display: "block", marginBottom: 6, letterSpacing: "0.3px" }

  return (
    <div style={{ minHeight: "100vh", background: C.bg, display: "flex", alignItems: "center", justifyContent: "center", padding: 20, fontFamily: "Archivo, system-ui, sans-serif" }}>
      ...
      <div style={{ width: "100%", maxWidth: 420, position: "relative", zIndex: 1 }}>
        ...
        <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 16, padding: 32, boxShadow: "0 4px 24px rgba(124,92,252,0.07)" }}>
          <div style={{ fontSize: 17, fontWeight: 700, color: C.text, marginBottom: 8, fontFamily: "Archivo, system-ui, sans-serif" }}>
          ...
          <button
            ...
            disabled={loading}
            style={{ width: "100%", background: loading ? "#B8A9F8" : C.accent, color: "#fff", border: "none", borderRadius: 10, padding: "13px 0", fontSize: 14, fontWeight: 700, cursor: loading ? "not-allowed" : "pointer", fontFamily: "Archivo, system-ui, sans-serif", marginBottom: 16 }}
          >
          ...
          <div style={{ textAlign: "center", fontSize: 13, color: C.muted }}>
            {mode === 'login' ? (
              <>New here? <button onClick={() => { setMode('signup'); setError(null); setMessage(null); }} style={{ background: "none", border: "none", color: C.accent, cursor: "pointer", fontWeight: 600, fontSize: 13, fontFamily: "Archivo, system-ui, sans-serif" }}>Set up your account</button></>
            ) : (
              <>Already set up? <button onClick={() => { setMode('login'); setError(null); setMessage(null); }} style={{ background: "none", border: "none", color: C.accent, cursor: "pointer", fontWeight: 600, fontSize: 13, fontFamily: "Archivo, system-ui, sans-serif" }}>Sign in</button></>
            )}
          </div>
        </div>
