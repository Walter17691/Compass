...
**Why Scenario 5's pre-seed is manual, not scripted:** Scenarios 1–4/6 only need an *empty* starting point (delete-and-done). Scenario 5 needs a *specific, realistic, internally-consistent* case state — the right allegation text, the right evidence entries, the right stage — for the tester's very first "what happened here" read to be genuine. Writing that via direct database inserts risks producing rows that don't match every computed/derived field the real Case Workspace screens expect (stage-derivation logic, `getNextStep`, guardrail inputs), which could hand a tester a subtly broken case and produce a false UAT finding that's actually a seeding bug, not a Compass bug. Building it once, by hand, through Compass's own real case-creation and investigation UI — the same flow any real HR user would use — guarantees the seeded case is exactly as valid as any real case, at the cost of ~10–15 minutes of manual setup before the (currently single) Scenario 5 session. That trade is worth it for one session; if a later UAT round needs Scenario 5 run many more times, revisit scripting it then, informed by having done it manually at least once first.
