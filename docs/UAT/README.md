...
| M | [M-PILOT-SESSION-PACK.md](./M-PILOT-SESSION-PACK.md) | *(Phase 8B)* Condensed, single-document facilitator/tester/observation/questionnaire pack for the Scenario 5 pilot only — everything needed live, without cross-referencing A–L mid-session |

Run sessions in the order Documents A→B naturally imply: read A once, use B every session, C+D are what you hand the tester, E+F are what you fill in during/after, and G/H/I are completed once real data exists. J/K/L (Phase 8A) sit in front of all of that — they make the programme actually executable: J says *where* UAT runs and *who's* in it, K says *exactly what to do* each session, L says whether you're actually ready to start. M (Phase 8B) is the pilot-day companion — print or open it alone for the actual pilot session; A–L stay the authoritative source if anything in M ever seems to disagree.
