...
| **Review draft content** | persisted `meeting.reviewDraft`, written by `transitionMeeting` `review_draft → review_draft` | was React state only, so re-entry regenerated and discarded edits | Phase 3B slice 2 | CONSISTENT (human UAT pending) |
| **Draft provenance** | `transcriptFingerprint` — FNV-1a over a documented canonicalisation of the persisted transcript | nothing; a count would not have proved content | Phase 3B slice 2 | CONSISTENT |
| **Draft conflict handling** | conflict suspends autosave and requires explicit retry; `STALE_STATUS` is a hard stop | n/a | Phase 3B slice 2 | CONSISTENT |
| **Employee-facing vs internal record** | `splitMeetingRecord` — one matcher, applied at generation, draft persistence, edit, save and signature | `record` was ONE string containing `## HR Advisor Notes`, so internal analysis was editable, confirmable and (on a heading variant) sendable — NEW-39 | 2026-09-25 | CONSISTENT |
| **Signed document source** | employee-facing half of the **persisted** record | was the whole record, cut by a bare exact-string `indexOf` | NEW-39 | CONSISTENT |
| **End-meeting gate** | none — End ends the meeting; gaps surface in Review as non-blocking advice | blocking quality-check modal + a second "Proceed anyway?" | NEW-40 | CONSISTENT |
| **Suggestion identity** | `suggestionKey` + `mergeSuggestions`, seen-set grows through the batch | seen-set built from `existing` only, so duplicates within one AI response both passed | NEW-41 | CONSISTENT |
| **User-facing legal assertions** | stated only with a source; a configured org policy is surfaced with attribution, otherwise neutral guidance | a hard-coded "must … at least 48 hours … (ERA 1999 s.10)" with no source, while two other surfaces deliberately refused to guess a number | NEW-42 / Phase 4A | CONSISTENT |
| **Case-requirement explanation** | classified per type and shown BEFORE Start/Schedule/Prepare, with the reason on every disabled control | explained only at persistence, after the work was done | NEW-43 / Phase 4B | CONSISTENT |
| **Standalone meeting persistence** | not implemented — `PARENT_REQUIRED` remains the backstop | n/a | **Phase 4C–4F** | KNOWN GAP |
...
| **Save & send (compound)** | one orchestration: `saveMeetingToCase()` → on success only → signature; `signId` attached via `transitionMeeting` `allowedFrom: [completed]` | signature used to run first and complete as a side effect | Phase 3B slice 1 UX — **human verified** 2026-09-25 | CONSISTENT |
